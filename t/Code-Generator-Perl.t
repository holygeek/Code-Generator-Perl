...
$generator->new_package(package => 'CorrectOrdering');
$generator->add(
    wheel_count_for => {
	car => 4,
	bicycle => 2,
...
ok($generator->create(), 'Generate CorrectOrdering.pm');
$expected = <<EOT;
package CorrectOrdering;
...
ok (compare_with_file($expected, 't/tmp/CorrectOrdering.pm'), 'Correct ordering');
...
ok($generator->create(), 'Generate UseNone.pm');
$expected = <<EOT;
package UseNone;
...
ok (compare_with_file($expected, 't/tmp/UseNone.pm'), 'Use none');
...
ok($generator->create(), 'Generate NewGeneratedBy');
$expected = <<EOT;
package $package_name;
