...
sub create {
	...
	_print_line($file, "package $package;");
	_print_line($file);

	my @uses;
	foreach my $package (@{$self->{use}}) {
		_print_line($file, "use $package;");
	}
	if (scalar @{$self->{use}}) {
		_print_line($file);
	}

	_print_line($file, '# You should never edit this file. '
			 . 'Everything in here is automatically');
	_print_line($file, '# generated by ' . $self->{generated_by} . '.');
	_print_line($file);

	foreach my $content (@{$self->{content}}) {
		_print_line($file, $content);
	}
